...
class ClientGUI(QMainWindow):
    ...
    def __init__(self, server_host, server_port):
        ...
        self.tray_icon.setContextMenu(tray_menu)  # Now it's safe to set the context menu
    ...
    def load_client_id(self):
        ...
        try:
            with open(CLIENT_CONFIG_FILE, "r") as file:
                ...
                return config.get("client_id")  # Use .get() for safety
    ...
    def show_login_dialog(self):
        ...
        while True:  # Keep asking until a valid ID is entered
            ...
            if ok and client_id.strip():
                ...
                self.connect_thread.start()
                break
            elif ok:  # User pressed OK, but didn't enter an ID
        ...
        while self.connected:
            try:
                ...
                if data["type"] == "initial_tasks":
                    ...
                    self.tasks.append(data["data"])  # Add the new task
                    self.update_signal.emit({"type": "tasks", "data": self.tasks})
                    ...
                    if 0 <= task_id_to_delete < len(self.tasks):
                        del self.tasks[task_id_to_delete]  # Delete the task
                        self.update_signal.emit({"type": "tasks", "data": self.tasks})
    ...
    def update_ui(self, update_data):
        if update_data["type"] == "tasks":
            ...
            for i, task in enumerate(update_data["data"]):
                ...
                # Store the task index as data within the list item
    ...
    def mark_task_completed(self):
        ...
        if selected_item:
            ...
            if 0 <= task_index < len(self.tasks):
                ...
                new_status = (
                    "Completed" if current_status != "Completed" else "In Progress"
                )  # Toggle status
                ...
                try:
                    update_message = json.dumps(
                        {"task_update": {"task_id": task_index, "status": new_status}}
                    )
                    ...
                    QMessageBox.critical(
                        self,
                        "Error",
                        "Failed to send task update. Check server connection.",
                    )
            else:
    ...
    def handle_notification(self, notification_data):
        ...
        self.tray_icon.showMessage(
            "New Notification", message, QSystemTrayIcon.MessageIcon.Information, 5000
        )  # 5 second display
        ...
        try:
            read_message = json.dumps(
                {"notification_read": notification_data["id"]}
            )
    ...
    def toggle_window_visibility(self):
        if self.isVisible():
            ...
            self.showNormal()  # Use showNormal() to restore from minimized state
            self.activateWindow()  # Bring to front

    def close_application(self):
        self.connected = False  # Set connected to False *before* closing
...
if __name__ == "__main__":
    ...
    client = ClientGUI("127.0.0.1", 5000)  # Use provided ID
    client.show()
    sys.exit(app.exec())
